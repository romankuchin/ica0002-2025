...
If all done correctly, `Public HA URLs` of `your-gh-username-1` should show you Agama app. Stopping HAProxy service on any single VM should not affect Agama service reachability.
...
## Task 7. Fix SLO metrics

Since now users won't use Nginx as reverse proxy to Agama app, but will use HAProxy instead. So need to expose the same stats as Nginx does for SLO calculations.
...
HAProxy can't write logs to a file, only to syslog, so make sure that Promtail can accept some logs from HAProxy: https://grafana.com/docs/loki/latest/send-data/promtail/configuration/#syslog. Add another job to static_configs list:
...
Promtail can listen only on TCP ports and making HAProxy to send logs via TCP is not too straitforward, but they provide decent [docs](https://www.haproxy.com/documentation/haproxy-configuration-tutorials/protocol-support/syslog-forwarding/#forward-haproxy-logs).

Make sure that your SLO dashboard now uses input from HAProxy.
