...
For this demo we will need to set up a few things first:
 ...
 4. User `backup` access to MySQL database; it will be configred in the following lab task 2
...
`backup.<your-domain>` as required in the lab 9.

Note: make sure to complete tasks 1 and 2 of the lab 10 before running any further command from
this demo.
...
Note that amount of bytes transferred is much less than the file size:
    ...
    ...
    sent 96 bytes  received 53 bytes  298.00 bytes/sec
    total size is 2,021  speedup is 13.56
...
data chunks are sent at all; only a few bytes (96 sent, 53 received in this example) are spent on
...
This command should print something similar to:
    ...
    ...
    sent 1,429 bytes  received 53 bytes  988.00 bytes/sec
...
changes together with `rsync` own protocol payload (1429 bytes) are still smaller than the actual
...
backups to make things more interesting:
    ...
    mysql -e 'TRUNCATE TABLE agama.item'
    rm agama.sql
...
If you refresh the Agama app page it now -- the list will be empty; all the precious customer data
is gone :(
...
Note that three additional files were created:
    ...
    duplicity-full-signatures.20251103T181141Z.sigtar.gz
    duplicity-full.20251103T181141Z.manifest
    duplicity-full.20251103T181141Z.vol1.difftar.gz
...
Note the `incremental` parameter. On the backup server three more files are created:
    ...
    duplicity-inc.20251103T181141Z.to.20251103T181808Z.manifest
    duplicity-inc.20251103T181141Z.to.20251103T181808Z.vol1.difftar.gz
    duplicity-new-signatures.20251103T181141Z.to.20251103T181808Z.sigtar.gz
...
**on the backup server**; your file names will differ):
    ...
    zless duplicity-full.20251103T181141Z.vol1.difftar.gz
    zless duplicity-inc.20251103T181141Z.to.20251103T181808Z.vol1.difftar.gz
