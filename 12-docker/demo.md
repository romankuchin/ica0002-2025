...
chosen not to conflict with another package in Debian and Ubuntu repositories called `docker`
(renamed to `wmdocker` recently); the latter has nothing to do with containers:
...
where "package" is the Docker image.

The action above has left some artifacts on the Docker host, namely the downloaded image:
...
We cannot do much with this `hello-world` app. So let's run another container:
 ...
 - `alpine` here stands for image name; this is an image based on
...
This is the shell inside the container! Let's look around there:
    ...
    uname -a
    > Linux 61cf0d96ff30 6.8.0-48-generic #48-Ubuntu ... Fri Sep 27 14:04:52 UTC 2024 x86_64 Linux

The first command will print the system identification -- the system running in the container as
seen by applications is Alpine Linux v3.22.
...
like `http://193.40.157.25:xx80/grafana`, and that required additional Grafana configuration in
`grafana.ini` file. This file is now located in the Docker container, and although it's possible to
change it dynamically -- there is a better way.
...
http://193.40.157.25:xxx80/grafana
